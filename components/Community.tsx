...
const items = [
  { key: 'github', label: 'GitHub', href: socialConfig.github, icon: GithubIcon },
  { key: 'whatsapp', label: 'WhatsApp Community', href: socialConfig.whatsapp, icon: WhatsappIcon },
  { key: 'instagram', label: 'Instagram', href: socialConfig.instagram, icon: InstagramIcon }
];
            ...
            {items.map((item) =>
              item.href ? (
                <a
                  ...
                  target="_blank"
                  rel="noreferrer"
                  className="btn-tactile flex items-center gap-2 rounded-full border border-hairline px-5 py-2.5 text-sm text-chrome-200 hover:border-signal-500/40 hover:text-signal-400"
                >
